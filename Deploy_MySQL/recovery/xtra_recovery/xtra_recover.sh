...
install_qpress(){
    if [ -a /usr/bin/qpress ];then
        print_message $WARNING "qpress工具已存在"
    else
        chmod 775 $BASE_DIR/qpress
        cp $BASE_DIR/qpress /usr/bin
    fi
}
...
xtrabackup_decompress(){
    $XTRABACKUP_PATH/xtrabackup --decompress --remove-original --target-dir=$BACKUP_FLIE_DIR/$TMP_BACKUP_NAME >> $BASE_DIR/$BACKUP_LOG 2>&1 || err_message "解压失败,可查看$BASE_DIR/$BACKUP_LOG 获取详细信息"
}

xtrabackup_prepare(){
    xtra_backup_dir=$1
    $XTRABACKUP_PATH/xtrabackup --prepare --target-dir=$xtra_backup_dir >> $BASE_DIR/$BACKUP_LOG 2>&1 || err_message "prepare执行失败,可查看$BASE_DIR/$BACKUP_LOG 获取详细信息"
}

check_diff_params(){
    source_innodb_data_file=$(cat $BACKUP_FLIE_DIR/$TMP_BACKUP_NAME/backup-my.cnf  |grep innodb_data_file_path|grep -v -E '^#'|awk -F "=" '{print $2}'|sed 's/[[:space:]]//g')
    ...
    source_innodb_page_size=$(cat $BACKUP_FLIE_DIR/$TMP_BACKUP_NAME/backup-my.cnf  |grep innodb_page_size|grep -v -E '^#'|awk -F "=" '{print $2}'|sed 's/[[:space:]]//g')
    ...
    source_server_id=$(cat $BACKUP_FLIE_DIR/$TMP_BACKUP_NAME/backup-my.cnf  |grep server_id|grep -v -E '^#'|awk -F "=" '{print $2}'|sed 's/[[:space:]]//g')
...
xtrabackup_backup(){
    xtra_backup_dir=$1
    $XTRABACKUP_PATH/xtrabackup --defaults-file=$CONFIG_FILE --move-back --target-dir=$xtra_backup_dir >> $BASE_DIR/$BACKUP_LOG 2>&1 || err_message "恢复失败,可查看$BASE_DIR/$BACKUP_LOG 获取详细信息"
}
...
xtrabackup_recovery(){
    print_message $INFO "安装解压工具qpress..."
    install_qpress
    print_message $INFO "解包xbstream文件..."
    xbstream_files
    print_message $INFO "使用qpress解压..."
    xtrabackup_decompress
    print_message $INFO "检查参数设置是否正确..."
    check_diff_params
    print_message $INFO "prepare..."
    xtrabackup_prepare $BACKUP_FLIE_DIR/$TMP_BACKUP_NAME
    print_message $INFO "停止mysql..."
    $STOP_CMD
    print_message $INFO "移动原数据文件..."
    move_datadir
    print_message $INFO "恢复中..."
    xtrabackup_backup $BACKUP_FLIE_DIR/$TMP_BACKUP_NAME
