...
change_master(){
  ...
  if [[ $MYSQL_VERSION > "8.0.25" ]];then
    /usr/local/mysql/bin/mysql -u $LOGIN_USER -p"${LOGIN_PASSWORD}" -P$LOGIN_PORT -S $LOGIN_SOCKET -e "RESET MASTER;CHANGE REPLICATION SOURCE TO SOURCE_HOST = '$SOURCE_HOST',SOURCE_PORT = $SOURCE_PORT,SOURCE_USER = '$SOURCE_USER',SOURCE_PASSWORD = '$SOURCE_PASSWORD',SOURCE_AUTO_POSITION = 1,MASTER_SSL = 1;START REPLICA;"
  else
    /usr/local/mysql/bin/mysql -u $LOGIN_USER -p"${LOGIN_PASSWORD}" -P$LOGIN_PORT -S $LOGIN_SOCKET -e "RESET MASTER;CHANGE MASTER TO MASTER_HOST = '$SOURCE_HOST',MASTER_PORT = $SOURCE_PORT,MASTER_USER = '$SOURCE_USER',MASTER_PASSWORD = '${SOURCE_PASSWORD}',MASTER_AUTO_POSITION = 1;START SLAVE;"
  fi
